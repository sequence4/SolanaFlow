    ...
    useEffect(() => {
        ...
        if (connected && publicKey) {
            iframeRef.current.contentWindow.postMessage(
                ...
                iframeSrc                                  // ← origin check (safe)
            );
        }
    }, [connected, publicKey, iframeSrc]);

    /* ───────── iframe → parent: handle connection requests ───────── */
    useEffect(() => {
        function handleIframeMsg(event: MessageEvent) {
            /* accept only messages coming from OUR iframe */
            if (event.source !== iframeRef.current?.contentWindow) return;
            const { type } = event.data || {};
            if (type === "REQUEST_WALLET_CONNECT") {
                if (!connected) connect();                 // triggers Phantom / Solflare, etc.
            }
        }
