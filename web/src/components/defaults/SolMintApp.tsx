...
// IDL will be loaded dynamically or passed as prop
// This is a fallback component that works without IDL
const solMintIdl: any = null
...
async function getProgram(
  ...
  if (!idl) {
    console.warn("[IDL] No IDL available - program methods will not be accessible")
    return null
  }
...
export default function SolMintApp() {
  ...
  const handleInitializeMint = async () => {
    ...
    try {
      ...
      if (!program) {
        console.error("[ERROR] Program IDL not available")
        toast({
          title: "Program Not Ready",
          description: "Waiting for program IDL to be generated. Please deploy the program first.",
          variant: "destructive"
        })
        return
      }
