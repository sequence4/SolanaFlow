...
exports.getContainerName = exports.startDeleteFileTask = exports.startUpdateFileTask = exports.startCreateFileTask = exports.startGetFileContentTask = exports.startGenerateFileTreeTask = exports.startDeleteProjectFolderTask = exports.deleteProjectFolder = void 0;
...
async function getProjectRootPath(projectId) {
    ...
    try {
        const result = await client.query('SELECT root_path FROM solanaproject WHERE id = $1', [projectId]);
...
const deleteProjectFolder = async (rootPath, taskId) => {
    const projectPath = path_1.default.join(appConfig_1.APP_CONFIG.ROOT_FOLDER, rootPath);
    if (!fs_1.default.existsSync(projectPath)) {
        await (0, taskUtils_1.updateTaskStatus)(taskId, 'finished', 'Project folder does not exist');
        return;
    }
    try {
        await fs_1.default.promises.rm(projectPath, { recursive: true, force: true });
        await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'Project folder deleted successfully');
    }
    catch (error) {
        console.error('Error deleting project folder:', error);
        await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', 'Failed to delete project folder');
    }
};
exports.deleteProjectFolder = deleteProjectFolder;
const startDeleteProjectFolderTask = async (rootPath, creatorId) => {
    const client = await database_1.default.connect();
    try {
        const taskId = (0, uuid_1.v4)();
        await client.query('INSERT INTO task (id, name, created_at, creator_id, status) VALUES ($1, $2, NOW(), $3, $4)', [taskId, 'Delete Project Folder', creatorId, 'doing']);
...
async function generateFileTree(dir, relativePath = '') {
...
async function createTempTask(operation, projectId, userId) {
    return await (0, taskUtils_1.createTask)(`Temp ${operation}`, userId, projectId);
}
async function generateFileTreeInContainer(containerName, rootPath, projectId, userId) {
    try {
        ...
        try {
            const output = await (0, projectUtils_1.runCommand)(command, '.', tempTaskId);
            ...
            if (allItems.some(item => item.path.includes('/programs/'))) {
                try {
                    ...
                    const programsOutput = await (0, projectUtils_1.runCommand)(programsCommand, '.', tempTaskId);
            ...
            const simplifiedOutput = await (0, projectUtils_1.runCommand)(simplifiedCommand, '.', tempTaskId);
...
const startGenerateFileTreeTask = async (projectId, rootPath, creatorId) => {
    try {
        const taskId = await (0, taskUtils_1.createTask)('Generate File Tree', creatorId, projectId);
        setImmediate(async () => {
            try {
                let fileTree = [];
                const containerQuery = await database_1.default.query('SELECT container_name FROM solanaproject WHERE id = $1', [projectId]);
                ...
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', treeResult);
            }
            catch (error) {
                console.error('Error generating file tree:', error);
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', 'Failed to generate file tree: ' + (error instanceof Error ? error.message : String(error)));
            }
        });
        return taskId;
    }
...
const startGetFileContentTask = async (projectId, filePath, creatorId) => {
    const taskId = await (0, taskUtils_1.createTask)('Get File Content', creatorId, projectId);
    setImmediate(async () => {
        try {
            const containerQuery = await database_1.default.query('SELECT container_name FROM solanaproject WHERE id = $1', [projectId]);
            const containerName = containerQuery.rows.length > 0 ? containerQuery.rows[0].container_name : null;
            ...
            if (containerName) {
                try {
                    ...
                    content = await (0, projectUtils_1.runCommand)(readCmd, '.', taskId);
                    ...
                    content = await fs_1.default.promises.readFile(fullPath, 'utf-8');
                }
            }
            else {
            ...
            await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', `Failed to read file: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    return taskId;
};
exports.startGetFileContentTask = startGetFileContentTask;
const startCreateFileTask = async (projectId, filePath, content, creatorId) => {
    const taskId = await (0, taskUtils_1.createTask)('Create File', creatorId, projectId);
    setImmediate(async () => {
        try {
            const containerQuery = await database_1.default.query('SELECT container_name FROM solanaproject WHERE id = $1', [projectId]);
            const containerName = containerQuery.rows.length > 0 ? containerQuery.rows[0].container_name : null;
            ...
            if (containerName) {
                try {
                    ...
                    if (dirPath && dirPath !== '.') {
                        ...
                        await (0, projectUtils_1.runCommand)(mkdirCmd, '.', taskId);
                    }
                    const writeCmd = `
...
EOF`;
                    ...
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File created successfully in container');
                }
                catch (containerError) {
                    ...
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File created successfully (local fallback)');
                }
            }
            else {
                ...
                await fs_1.default.promises.mkdir(path_1.default.dirname(fullPath), { recursive: true });
                await fs_1.default.promises.writeFile(fullPath, content, 'utf-8');
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File created successfully');
            }
        }
        catch (error) {
            console.error('Error creating file:', error);
            await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', `Failed to create file: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    return taskId;
};
exports.startCreateFileTask = startCreateFileTask;
const startUpdateFileTask = async (projectId, filePath, content, creatorId) => {
    const taskId = await (0, taskUtils_1.createTask)('Update File', creatorId, projectId);
    setImmediate(async () => {
        try {
            const containerQuery = await database_1.default.query('SELECT container_name FROM solanaproject WHERE id = $1', [projectId]);
            const containerName = containerQuery.rows.length > 0 ? containerQuery.rows[0].container_name : null;
...
EOF`;
                    ...
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File updated successfully in container');
                }
                catch (containerError) {
                    ...
                    const fullPath = path_1.default.join(appConfig_1.APP_CONFIG.ROOT_FOLDER, projectRootPath, filePath);
                    await fs_1.default.promises.writeFile(fullPath, content, 'utf-8');
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File updated successfully (local fallback)');
                }
            }
            else {
                ...
                const fullPath = path_1.default.join(appConfig_1.APP_CONFIG.ROOT_FOLDER, projectRootPath, filePath);
                await fs_1.default.promises.writeFile(fullPath, content, 'utf-8');
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File updated successfully');
            }
        }
        catch (error) {
            console.error('Error updating file:', error);
            await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', `Failed to update file: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    return taskId;
};
exports.startUpdateFileTask = startUpdateFileTask;
const startDeleteFileTask = async (projectId, filePath, creatorId) => {
    const taskId = await (0, taskUtils_1.createTask)('Delete File', creatorId, projectId);
    setImmediate(async () => {
        try {
            const containerQuery = await database_1.default.query('SELECT container_name FROM solanaproject WHERE id = $1', [projectId]);
            const containerName = containerQuery.rows.length > 0 ? containerQuery.rows[0].container_name : null;
            ...
            if (containerName) {
                try {
                    ...
                    await (0, projectUtils_1.runCommand)(deleteCmd, '.', taskId);
                    console.log(`Successfully deleted file from container: ${filePath}`);
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File deleted successfully from container');
                }
                catch (containerError) {
                    ...
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File deleted successfully (local fallback)');
                }
            }
            else {
                ...
                const fullPath = path_1.default.join(appConfig_1.APP_CONFIG.ROOT_FOLDER, projectRootPath, filePath);
                await fs_1.default.promises.unlink(fullPath);
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'File deleted successfully');
            }
        }
        catch (error) {
            console.error('Error deleting file:', error);
            await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', `Failed to delete file: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    return taskId;
};
exports.startDeleteFileTask = startDeleteFileTask;
const getContainerName = async (projectId) => {
    try {
        const { rows } = await database_1.default.query('SELECT container_name FROM solanaproject WHERE id = $1', [projectId]);
        if (rows.length > 0) {
