...
async function getContainerHostPort(containerName, containerPort = 3000, taskId) {
    try {
...
async function runCommand(command, cwd, taskId, options = {}) {
    return new Promise((resolve, reject) => {
        ...
        (0, child_process_1.exec)(command, { cwd }, async (error, stdout, stderr) => {
            let result = '';
            ...
            if (error) {
                ...
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', result);
                return reject(new Error(result));
            }
            if (!options.skipSuccessUpdate) {
                if (hasWarning(stdout) || (stderr && hasWarning(stderr))) {
                    result = `Warning detected:\n\nStdout: ${stdout.trim()}\n\nStderr: ${stderr.trim()}`;
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'warning', result);
                }
                else {
                    result = `Success`;
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', result);
                }
            }
            resolve(stdout.trim());
        });
...
;
async function compileTs(tsFileName, compileCwd, distFolder = "dist") {
    ...
    if (!fs_1.default.existsSync(jsFilePath)) {
        ...
        await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', msg);
        throw new Error(msg);
    }
    const compiledJs = fs_1.default.readFileSync(jsFilePath, "utf8");
    console.log(`Read compiled JS from: ${jsFilePath}`);
    await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', `Compiled ${tsFileName} -> ${jsFileName}`);
    return compiledJs;
}
const startAnchorInitTask = async (projectId, rootPath, projectName, creatorId) => {
    const taskId = await (0, taskUtils_1.createTask)('Anchor Init', creatorId, projectId);
    setImmediate(async () => {
        try {
            ...
            await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', `Error: ${error.message}`);
        }
    });
    return taskId;
};
exports.startAnchorInitTask = startAnchorInitTask;
const startSetClusterTask = async (projectId, creatorId) => {
    const taskId = await (0, taskUtils_1.createTask)('Anchor Config Set Devnet', creatorId, projectId);
    setImmediate(async () => {
        try {
            const containerName = await getContainerName(projectId);
            if (!containerName) {
...
const getBuildArtifactTask = async (projectId) => {
    try {
        const rootPath = await (0, fileUtils_1.getProjectRootPath)(projectId);
...
const startAnchorBuildTask = async (projectId, creatorId) => {
    const taskId = await (0, taskUtils_1.createTask)('Anchor Build', creatorId, projectId);
...
`;
            ...
            try {
                await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'doing', 'Anchor build in progress...');
                console.log(`Copying build script to container ${containerName}...`);
                await runCommand(`docker cp ${buildScriptPath} ${containerName}:/tmp/build.sh`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
                console.log(`Making build script executable...`);
                await runCommand(`docker exec ${containerName} chmod +x /tmp/build.sh`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
                console.log(`Executing build script in container ${containerName}...`);
                const buildOutput = await runCommand(`docker exec ${containerName} /bin/bash /tmp/build.sh`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
                const transformedRootPath = rootPath.replace(/-/g, '_');
                const soFileCheck = await runCommand(`docker exec ${containerName} /bin/bash -c "if [ -f /usr/src/${rootPath}/target/deploy/${transformedRootPath}.so ]; then echo 'BUILD_SUCCESS: .so file was created'; else echo 'BUILD_FAILURE: .so file was NOT created'; fi"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
                try {
                ...
                if (soFileCheck.includes('BUILD_SUCCESS')) {
                    await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'succeed', `Build completed successfully. .so file was created.`);
                }
                else {
                    ...
                    await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', fullBuildError);
                }
            }
            catch (buildError) {
                console.error(`Anchor build failed with error: ${buildError.message}`);
                await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', `Anchor build failed: ${buildError.message || 'Unknown build error occurred'}`);
                try {
                    fs_1.default.unlinkSync(buildScriptPath);
                }
                catch (cleanupError) {
            ...
            await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', `Error: ${error.message}`);
        }
    });
    return taskId;
};
exports.startAnchorBuildTask = startAnchorBuildTask;
const startAnchorDeployTask = async (projectId, creatorId, ephemeralPubkey) => {
    const taskId = await (0, taskUtils_1.createTask)('Anchor Deploy', creatorId, projectId);
    let programId = null;
    const sanitizedTaskId = taskId.trim().replace(/,$/, '');
    ...
    setImmediate(async () => {
        try {
            ...
            if (ephemeralPubkey) {
                ...
                try {
                    const fileStats = fs_1.default.statSync(walletPath);
                    console.log(`[DEPLOY_DEBUG] Key file exists: ${walletPath}, size: ${fileStats.size} bytes`);
                    const keyContent = fs_1.default.readFileSync(walletPath, 'utf8');
                ...
                try {
                    ...
                    if (walletLineMatch) {
                        ...
                        if (walletPath.includes('id.json') || walletPath.includes('~')) {
                            ...
                            try {
                                const modifiedToml = anchorTomlContent.replace(/wallet\s*=\s*["'][^"']+["']/, `wallet = "${containerWalletPath}"`);
        ...
        --provider.cluster devnet`;
            ...
            const result = await runCommand(deployCmd, '.', sanitizedTaskId, { skipSuccessUpdate: true }).catch(async (error) => {
                ...
                await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', errorResult);
            ...
            await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'succeed', successResult);
        }
        catch (error) {
            console.error('Error during deployment:', sanitizedTaskId, error);
            const errorResult = JSON.stringify({
...
const startAnchorTestTask = async (projectId, creatorId) => {
    const taskId = await (0, taskUtils_1.createTask)('Anchor Test', creatorId, projectId);
    const sanitizedTaskId = taskId.trim().replace(/,$/, '');
    setImmediate(async () => {
        try {
            const containerName = await getContainerName(projectId);
...
const startCustomCommandTask = async (projectId, creatorId, commandType, functionName, parameters, ephemeralPubkey) => {
    const taskId = await (0, taskUtils_1.createTask)(commandType === 'runFunction' ? `Run Function: ${functionName}` : commandType, creatorId, projectId);
    setImmediate(async () => {
        try {
            const containerName = await getContainerName(projectId);
            if (!containerName) {
            ...
            if (commandType === 'runFunction' && functionName) {
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'doing', `Executing function ${functionName}...`);
                try {
                    ...
                    try {
                        ...
                        await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', JSON.stringify(parsedResult));
                    }
                    catch (parseError) {
                        const wrappedResult = { message: output };
                        await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', JSON.stringify(wrappedResult));
                    }
                }
                catch (error) {
                    console.error(`Error executing function:`, error);
                    await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', `Error executing function: ${error.message}`);
                }
            }
            else {
                const rootPath = await (0, fileUtils_1.getProjectRootPath)(projectId);
...
const startInstallPackagesTask = async (projectId, creatorId, _packages) => {
    const taskId = await (0, taskUtils_1.createTask)('Install NPM Packages', creatorId, projectId);
    setImmediate(async () => {
        try {
            const containerName = await getContainerName(projectId);
            if (!containerName) {
...
const startInstallNodeDependenciesTask = async (projectId, creatorId, packages, targetDir = 'app') => {
    const taskId = await (0, taskUtils_1.createTask)('Install Node Dependencies', creatorId, projectId);
    ...
    setImmediate(async () => {
        try {
            if (packages.length === 0) {
                ...
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', 'No packages to install');
                return;
            }
            const containerName = await getContainerName(projectId);
            if (!containerName) {
            ...
            const rootPathResult = await database_1.default.query('SELECT name FROM solanaproject WHERE id = $1', [projectId]);
            let rootPath = '';
            ...
            await (0, taskUtils_1.updateTaskStatus)(taskId, 'doing', `Installing ${packages.join(', ')} in ${targetDir}...`);
            ...
            try {
                ...
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'succeed', `Successfully installed dependencies in container ${containerName} (${targetDir})`);
            }
            catch (error) {
                console.error(`Failed to install packages in container. Error:`, error);
                await (0, taskUtils_1.updateTaskStatus)(taskId, 'failed', `Error installing dependencies: ${error.message}`);
            }
        }
        catch (error) {
...
const startCreateProjectDirectoryTask = async (creatorId, rootPath, projectId, projectDesc = 'A React application') => {
    if (!projectId) {
    ...
    const taskId = await (0, taskUtils_1.createTask)('Create Project Directory', creatorId, projectId);
    const sanitizedTaskId = taskId.trim().replace(/,$/, '');
    ...
    setImmediate(async () => {
      ...
      `;
            ...
            try {
                ...
                const updateResult = await database_1.default.query('UPDATE solanaproject SET container_name = $1, container_url = $2 WHERE id = $3', [containerName, containerUrl, projectId]);
                ...
                const verifyResult = await database_1.default.query('SELECT container_name, container_url FROM solanaproject WHERE id = $1', [projectId]);
...
EOF`;
            ...
            await runCommand(`docker exec ${containerName} bash -c "cd /usr/src/${rootPath}/app && npm install"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            const readMeCmd = `
...
EOF`;
            ...
            await runCommand(`docker exec ${containerName} ls -la /usr/src/${rootPath}`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            await runCommand(`docker exec ${containerName} mkdir -p -v /usr/src/${rootPath}/server/src`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            await runCommand(`docker exec ${containerName} ls -la /usr/src/${rootPath}`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            await runCommand(`docker exec ${containerName} ls -ld /usr/src/${rootPath}`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            await runCommand(`docker exec ${containerName} ls -la /usr/src/${rootPath}/server || echo "Server directory not found"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
...
EOF`;
            ...
            await runCommand(`docker exec ${containerName} ls -la /usr/src/${rootPath}/server || echo "Server directory not found after package.json"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            await runCommand(`docker exec ${containerName} cat /usr/src/${rootPath}/server/package.json || echo "Could not read package.json"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
...
EOF`;
            ...
            await runCommand(`docker exec ${containerName} ls -la /usr/src/${rootPath}/server/src || echo "Server src directory not found"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            await runCommand(`docker exec ${containerName} bash -c "cd /usr/src/${rootPath}/server && npm install"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
...
EOF`;
            ...
            await runCommand(`docker inspect ${containerName} | grep -A 20 "Mounts"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
...
EOF`;
      ...
      `, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            await runCommand(`docker inspect --format='{{.Config.Image}}' ${containerName} && docker history $(docker inspect --format='{{.Config.Image}}' ${containerName})`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            console.log(`[DEBUG_CONTAINER] Final verification of directory structure for container ${containerName}...`);
            const finalVerification = await runCommand(`docker exec ${containerName} bash -c "
      ...
      `, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            if (finalVerification.includes('Server directory exists?\nNO')) {
                ...
                await runCommand(`docker exec ${containerName} bash -c "
      ...
      `;
            ...
            await runCommand(`docker exec ${containerName} bash -c "ps aux | grep 'react-scripts start' | grep -v grep || echo 'CRA process not found!'"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            console.log(`[DEBUG_CONTAINER] Checking CRA startup logs...`);
            await runCommand(`docker exec ${containerName} bash -c "cat /usr/src/${rootPath}/app/cra-startup.log || echo 'No startup log found'"`, '.', sanitizedTaskId, { skipSuccessUpdate: true });
            ...
            if (isServerReady) {
                ...
                await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'succeed', `Project created successfully! Anchor project in /usr/src/${rootPath}, CRA in /usr/src/${rootPath}/app, Express in /usr/src/${rootPath}/server, container: ${containerName}`);
                console.log(`[DEBUG_CONTAINER] Task marked as succeed. taskId=${sanitizedTaskId}`);
                try {
                    const finalCheckResult = await database_1.default.query('SELECT container_url FROM solanaproject WHERE id = $1', [projectId]);
                ...
                await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'succeed', `Project created! Container: ${containerName}. Anchor project in /usr/src/${rootPath}. Note: CRA dev server is still starting up and may need a few more moments.`);
                console.log(`[DEBUG_CONTAINER] Task marked as succeed. taskId=${sanitizedTaskId}`);
            }
        }
        catch (error) {
            console.error('[DEBUG_CONTAINER] Error creating project:', error);
            await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', `Error creating project directory: ${error.message}`);
...
const closeProjectContainer = async (projectId, creatorId, commitBeforeClose = false, removeContainer = false) => {
    const taskId = await (0, taskUtils_1.createTask)('Close Project Container', creatorId, projectId);
    const sanitizedTaskId = taskId.trim().replace(/,$/, '');
    setImmediate(async () => {
        try {
            const containerName = await getContainerName(projectId);
            ...
            if (removeContainer) {
                ...
                await database_1.default.query('UPDATE solanaproject SET container_name = NULL WHERE id = $1', [projectId]);
            }
            else {
                await runCommand(`docker stop ${containerName}`, '.', sanitizedTaskId);
                console.log(`Container ${containerName} stopped successfully`);
            }
            await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'succeed', `Container ${containerName} for project ${projectId} ${removeContainer ? 'stopped and removed' : 'stopped'} successfully`);
        }
        catch (error) {
            console.error(`Error closing project container:`, error);
            await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', `Error closing project container: ${error.message}`);
        }
    });
    return sanitizedTaskId;
};
exports.closeProjectContainer = closeProjectContainer;
const startProjectContainer = async (projectId, creatorId, rootPath) => {
    const taskId = await (0, taskUtils_1.createTask)('Start Project Container', creatorId, projectId);
    const sanitizedTaskId = taskId.trim().replace(/,$/, '');
    ...
    setImmediate(async () => {
        try {
          ...
          `;
                    ...
                    if (isServerReady) {
                        await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'succeed', `Existing container ${existingContainer} started successfully and CRA server is ready`);
                    }
                    else {
                        await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'succeed', `Existing container ${existingContainer} started successfully. CRA server is still starting up.`);
                    }
                }
                else if (containerStatus.toLowerCase().includes('up')) {
                    console.log(`Container ${existingContainer} is already running`);
                    await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'succeed', `Container ${existingContainer} is already running`);
                }
                else {
                    if (!rootPath) {
                        await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', `Container ${existingContainer} not found and rootPath not provided to create a new one`);
                        return;
                    }
                ...
                if (!rootPath) {
                    await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', `No container found for project ${projectId} and rootPath not provided to create a new one`);
                    return;
                }
                newContainerName = `userproj-${projectId}-${Date.now()}`;
            ...
            await (0, taskUtils_1.updateTaskStatus)(sanitizedTaskId, 'failed', `Error starting project container: ${error.message}`);
        }
    });
    return existingContainer ?? newContainerName ?? `failed-container-${projectId}`;
};
exports.startProjectContainer = startProjectContainer;
async function createNewContainer(containerName, projectId, taskId) {
  ...
  `;
    ...
    const containerUrl = `http://localhost:${hostPort}`;
    await database_1.default.query('UPDATE solanaproject SET container_name = $1, container_url = $2 WHERE id = $3', [containerName, containerUrl, projectId]);
    const rootPathResult = await database_1.default.query('SELECT name FROM solanaproject WHERE id = $1', [projectId]);
    let rootPath = '';
    if (rootPathResult.rows.length > 0) {
        rootPath = (0, stringUtils_1.normalizeProjectName)(rootPathResult.rows[0].name);
...
async function waitForServerReady(containerName, maxAttempts = 30, delayMs = 1000) {
    console.log(`Waiting for CRA server to be ready in container ${containerName}...`);
    const projectIdResult = await database_1.default.query('SELECT id FROM solanaproject WHERE container_name = $1', [containerName]);
...
async function runUserProjectCode(projectId, taskId, functionName, parameters, ephemeralPubkey) {
    const containerName = await getContainerName(projectId);
    if (!containerName) {
        throw new Error(`No container found for project ${projectId}`);
    }
