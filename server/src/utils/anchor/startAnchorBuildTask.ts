...
export const startAnchorBuildTask = async (
  ...
  setImmediate(async () => {
    try {
      ...
      // Keypair is already in the container from code generation at /usr/src/target/deploy/
      const containerKeyPathCheck = `/usr/src/target/deploy/${programName}-keypair.json`;

      // Verify the keypair exists in the container
      const keypairExistsCmd = `docker exec ${containerName} test -f ${containerKeyPathCheck} && echo "exists" || echo "missing"`;
      const keypairExists = await runCommand(keypairExistsCmd, '.', projectId, { skipSuccessUpdate: true });

      if (keypairExists.trim() !== 'exists') {
        throw new Error(`Program keypair not found in container at ${containerKeyPathCheck}. Run code generation first.`);
      }

      // Read and validate the keypair from container
      const readKeypairCmd = `docker exec ${containerName} cat ${containerKeyPathCheck}`;
      const secretJson = await runCommand(readKeypairCmd, '.', projectId, { skipSuccessUpdate: true });
      const secretArr: number[] = JSON.parse(secretJson);

      if (!Array.isArray(secretArr) || secretArr.length !== 64) {
        throw new Error(
          `Invalid keypair format in container (expected 64‑byte array)`,
        );
      }
      ...
      if (derivedPubkey !== programId) {
        throw new Error(
          `Keypair pubkey ${derivedPubkey} ≠ expected Program ID ${programId}`,
        );
      }

      // Ensure keypair is in the project-specific location for Anchor
      ...
      // Copy keypair from warm cache to project directory if not already there
      await runCommand(
        `docker exec ${containerName} bash -c 'if [ ! -f ${containerKeyPath} ]; then cp ${containerKeyPathCheck} ${containerKeyPath}; fi'`,
        '.',
        projectId,
        { skipSuccessUpdate: true },
      );
      console.log(`[BUILD] Ensured keypair is available at ${containerKeyPath} for program ${programId}`);
