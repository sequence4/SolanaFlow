...
/**
 * Returns a Keypair that the backend uses to pay transaction fees.
 *
 * ① Tries SERVER_FEE_PAYER env (JSON array of 64 ints).  
 * ② Falls back to < WALLET_FOLDER >/server_fee_payer.json.  
 * ③ If neither exists/valid ➜ generates one, stores it, returns it.
 */
export function getServerFeePayer(): Keypair {
  ...
  try {
    ...
    console.log(
      ...
      `Copy its contents to SERVER_FEE_PAYER env for production.`,
    );
  } catch (e) {
